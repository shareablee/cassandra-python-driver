...
from tests.integration.cqlengine.base import BaseCassEngTestCase, IntegerParititionModel, IntegerParititionModel_
...
class ContextQueryConnectionTests(BaseCassEngTestCase):
    ...
    def setUpClass(cls):
        ...
        with ContextQuery(IntegerParititionModel, connection='cluster') as tm:
            sync_table(tm)
    ...
    def test_context_connection_priority(self):
        ...
        IntegerParititionModel.__connection__ = 'cluster'
        with ContextQuery(IntegerParititionModel) as tm:
            tm.objects.create(partition=1, cluster=1)

        # ContextQuery connection should have priority over default one
        with ContextQuery(IntegerParititionModel, connection='fake_cluster') as tm:
        ...
        IntegerParititionModel.__connection__ = None
    ...
    def test_context_connection_with_keyspace(self):
        ...
        # ks2 doesn't exist
        with ContextQuery(IntegerParititionModel, connection='cluster', keyspace='ks2') as tm:
...
class ManagementConnectionTests(BaseCassEngTestCase):
    ...
    def test_create_drop_table(self):
        ...
        # No connection (default is fake)
        with self.assertRaises(NoHostAvailable):
            sync_table(IntegerParititionModel)

        # Explicit connections
        sync_table(IntegerParititionModel, connections=self.conns)

        # Explicit drop
        drop_table(IntegerParititionModel, connections=self.conns)
        ...
        # No connection (default is fake)
        with self.assertRaises(NoHostAvailable):
            drop_table(IntegerParititionModel)
...
class BatchQueryConnectionTests(BaseCassEngTestCase):
    ...
    def setUpClass(cls):
        ...
        create_keyspace_simple('ks1', 1)
        sync_table(IntegerParititionModel)
        sync_table(IntegerParititionModel_)
    ...
    def test_basic_batch_query(self):
        ...
        with self.assertRaises(NoHostAvailable):
            with BatchQuery() as b:
                IntegerParititionModel.objects.batch(b).create(partition=1, cluster=1)
    ...
    def test_batch_query_different_connection(self):
        ...
        IntegerParititionModel.__connection__ = 'cluster'
        IntegerParititionModel_.__connection__ = 'cluster2'

        with self.assertRaises(CQLEngineException):
            with BatchQuery() as b:
                IntegerParititionModel.objects.batch(b).create(partition=1, cluster=1)
                IntegerParititionModel_.objects.batch(b).create(partition=1, cluster=1)

        IntegerParititionModel.__connection__ = None
        IntegerParititionModel_.__connection__ = None
        ...
        with ContextQuery(IntegerParititionModel, IntegerParititionModel_, connection='cluster') as (tm, atm):
    ...
    def test_batch_query_connection_override(self):
        ...
        with self.assertRaises(CQLEngineException):
            with BatchQuery(connection='cluster') as b:
                IntegerParititionModel.batch(b).using(connection='test').save()

        with self.assertRaises(CQLEngineException):
            with BatchQuery(connection='cluster') as b:
                IntegerParititionModel.using(connection='test').batch(b).save()
...
class UsingDescriptorTests(BaseCassEngTestCase):
    ...
    def _reset_data(self):
        ...
        for ks in self.keyspaces:
            create_keyspace_simple(ks, 1, connections=self.conns)
        sync_table(IntegerParititionModel, keyspaces=self.keyspaces, connections=self.conns)
    ...
    def test_keyspace(self):
        ...
        with ContextQuery(IntegerParititionModel, connection='cluster') as tm:
            ...
            with self.assertRaises(IntegerParititionModel.DoesNotExist):
        ...
        with self.assertRaises(NoHostAvailable):
            IntegerParititionModel.objects.using(keyspace='ks2').get(partition=1, cluster=1)

        obj2 = IntegerParititionModel.objects.using(connection='cluster', keyspace='ks2').get(partition=1, cluster=1)
        self.assertEqual(obj2.count, 2)

        # Update test
        IntegerParititionModel.objects(partition=2, cluster=2).using(connection='cluster', keyspace='ks2').update(count=5)
        obj3 = IntegerParititionModel.objects.using(connection='cluster', keyspace='ks2').get(partition=2, cluster=2)
        self.assertEqual(obj3.count, 5)

        IntegerParititionModel.objects(partition=2, cluster=2).using(connection='cluster', keyspace='ks2').delete()
        with self.assertRaises(IntegerParititionModel.DoesNotExist):
            IntegerParititionModel.objects.using(connection='cluster', keyspace='ks2').get(partition=2, cluster=2)
    ...
    def test_connection(self):
        ...
        # Model class
        with self.assertRaises(NoHostAvailable):
            IntegerParititionModel.objects.create(partition=1, cluster=1)

        IntegerParititionModel.objects.using(connection='cluster').create(partition=1, cluster=1)
        IntegerParititionModel.objects(partition=1, cluster=1).using(connection='cluster').update(count=2)
        obj1 = IntegerParititionModel.objects.using(connection='cluster').get(partition=1, cluster=1)
        ...
        with self.assertRaises(IntegerParititionModel.DoesNotExist):
            IntegerParititionModel.objects.using(connection='cluster').get(partition=1, cluster=1)
